...
PHP: Versi yang sesuai dengan kebutuhan proyek Anda (tercantum di composer.json).
Database: MySQL, PostgreSQL, atau database lain yang didukung Laravel.
Composer: Untuk mengelola dependensi PHP.
(Opsional) Node.js & npm/yarn: Jika Anda perlu mengompilasi aset frontend (CSS/JS) menggunakan Vite atau Mix.
Meskipun ada file railway.json yang mungkin digunakan untuk platform Railway, itu tidak menghalangi Anda untuk melakukan deployment di VPS. Anda hanya perlu mengkonfigurasi VPS Anda dengan benar agar sesuai dengan kebutuhan aplikasi Laravel Anda.
Proses deployment ke VPS biasanya melibatkan langkah-langkah seperti:
Menyiapkan server (instalasi software di atas).
Mengunggah kode proyek Anda.
Mengkonfigurasi web server (misalnya, virtual host Nginx).
Mengatur file .env untuk lingkungan produksi.
Menjalankan composer install --optimize-autoloader --no-dev.
Menjalankan npm install && npm run build (jika menggunakan Vite/Mix).
Menjalankan migrasi database (php artisan migrate --force).
Mengatur permission folder (storage, bootstrap/cache).
(Opsional) Mengkonfigurasi Supervisor untuk queue worker, dll.
Jadi, ya, deployment di VPS sangat memungkinkan untuk proyek Laravel Anda.